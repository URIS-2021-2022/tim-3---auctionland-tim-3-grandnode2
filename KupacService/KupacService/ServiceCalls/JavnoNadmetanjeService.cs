...
namespace KupacService.ServiceCalls
{
    public class JavnoNadmetanjeService : IJavnoNadmetanjeService
    {
        ...
        public async Task<List<JavnoNadmetanjeDto>> GetJavnaNadmetanjaByKupacId(Guid kupacId)
        {
            try
            {
                using var httpClient = new HttpClient();
                Uri url = new Uri($"{ configuration["Services:JavnoNadmetanjeService"] }api/JavnaNadmetanjaLicitacija/" + kupacId);
